...
public class MacroButton extends MotifButton
    {
    ...
    public MacroButton(SeqUI sequi, MotifUI motifui, MacroUI owner, int at)
        {
        ...
        addMouseMotionListener(new MouseMotionAdapter()
            ...
            public void mouseDragged(MouseEvent e)
                {
                if (MacroButton.this.isEnabled())
                    {
                    dragCount++;
                    if (dragCount == MAX_DRAG_COUNT)
                        getTransferHandler().exportAsDrag(MacroButton.this, e, TransferHandler.MOVE);
                    }
                }
            });
...
/*
  ...
  try 
  {
  MacroClip clip = ((MacroClip)(owner.getMotif().getPlayingClip()));
  if (clip != null) 
  {
  ArrayList<MacroClip.Node> playing = clip.getNextClips();
  for(MacroClip.Node play : playing)
  {
...
/*
  MacroClip ownerClip = (MacroClip)(ownerMotif.getPlayingClip());
  if (ownerClip != null)
  {
  for(MacroClip.Node node : ownerClip.getPlayingClips())
  {
  if (node.index == at)
  {
  playing = true;
  ...
  if (!playing)
  {
  for(MacroClip.Node node : ownerClip.getNextClips())
  {
  if (node.index == at)
  {
  next = true;
