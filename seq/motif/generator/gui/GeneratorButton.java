...
public class GeneratorButton extends MotifButton
    {
    ...
    // Make a "Blank" button
    public GeneratorButton(SeqUI sequi, GeneratorUI owner, int at)   
        {
        super();
    ...
    public GeneratorButton(SeqUI sequi, MotifUI motifui, GeneratorUI owner, int at)
        {
        ...
        addActionListener(new ActionListener()
            ...
            public void actionPerformed(ActionEvent e)
                {
                dragCount = 0;
                owner.select(GeneratorButton.this);
                }
            });
        ...
        addMouseMotionListener(new MouseMotionAdapter()
            ...
            public void mouseDragged(MouseEvent e)
                {
                if (GeneratorButton.this.isEnabled())
                    {
                    dragCount++;
                    if (dragCount == MAX_DRAG_COUNT)
                        getTransferHandler().exportAsDrag(GeneratorButton.this, e, TransferHandler.MOVE);
                    }
                }
            });
...
/*
  ...
  try 
  {
  GeneratorClip clip = ((GeneratorClip)(owner.getMotif().getPlayingClip()));
  if (clip != null) 
  {
  ArrayList<GeneratorClip.Node> playing = clip.getNextClips();
  for(GeneratorClip.Node play : playing)
  {
...
/*
  GeneratorClip ownerClip = (GeneratorClip)(ownerMotif.getPlayingClip());
  if (ownerClip != null)
  {
  for(GeneratorClip.Node node : ownerClip.getPlayingClips())
  {
  if (node.index == at)
  {
  playing = true;
  ...
  if (!playing)
  {
  for(GeneratorClip.Node node : ownerClip.getNextClips())
  {
  if (node.index == at)
  {
  next = true;
