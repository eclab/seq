...
public class FunctionInspector extends JPanel
    ...
    public class DelayInspector extends SubInspector
		...
		public DelayInspector()
            ...
            */
			...
			build(new String[] { "", "Original", /*"Initial",*/ "Later", "Cut", "Num Delays"}, 
				new JComponent[] 
					{
					null,
					original,
    ...
    public class NoiseInspector extends SubInspector
		...
		public NoiseInspector()
			...
			distMin = new SmallDial((func.getDistMin() + 64) / 128.0)
				{
				protected String map(double val) { return String.valueOf((int)(val * 128 - 64)); }
				public double getValue() 
					{ 
					ReentrantLock lock = seq.getLock();
					lock.lock();
					try { return (func.getDistMin() + 64) / 128.0; }
					finally { lock.unlock(); }
					}
				public void setValue(double val) 
					{ 
					...
					ReentrantLock lock = seq.getLock();
					lock.lock();
					try { func.setDistMin((int)(val * 128 - 64)); }
					finally { lock.unlock(); }
					}
				};

			distMax = new SmallDial((func.getDistMax() + 64) / 128.0)
				{
				protected String map(double val) { return String.valueOf((int)(val * 128 - 64)); }
				public double getValue() 
					{ 
					ReentrantLock lock = seq.getLock();
					lock.lock();
					try { return (func.getDistMax() + 64) / 128.0; }
					finally { lock.unlock(); }
					}
				public void setValue(double val) 
					{ 
					...
					ReentrantLock lock = seq.getLock();
					lock.lock();
					try { func.setDistMax((int)(val * 128 - 64)); }
					finally { lock.unlock(); }
					}
				};
			...
			build(new String[] { "", "Type", "Value/Note/MSB", "LSB", "Min", "Max", "Rate"}, 
				new JComponent[] 
					{
					null,
					parameterType,
					...
					distMin.getLabelledDial("0.0000"),
					distMax.getLabelledDial("0.0000"),
					rate
					});
			}
		...
		public void revise() 
			...
			if (distMin != null) distMin.redraw();
			if (distMax != null) distMax.redraw();
