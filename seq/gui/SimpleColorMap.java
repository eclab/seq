...
public class SimpleColorMap 
    ...
    static final int COLOR_DISCRETIZATION = 128;
    ...
    ArrayList[] colorCache = new ArrayList[COLOR_DISCRETIZATION];
    ...
    */
    ...
    public void setLevels(double minLevel, double maxLevel, Color minColor, Color maxColor)
        ...
        // reset cache
        // (the slower cache)
        for(int x=0;x<COLOR_DISCRETIZATION;x++) colorCache[x] = new ArrayList();
    ...
    public Color getColor(double level)
        ...
        if (colors != null && level >= 0 && level < colors.length)
            ...
            ArrayList colors = colorCache[(int)(interpolation * (COLOR_DISCRETIZATION-1))];
            for(Object color : colors)	// int x=0;x<colors.numObjs;x++)
                {
                Color c = (Color)(color);		// colors.objs[x]);
