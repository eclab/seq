...
public class Heap implements java.io.Serializable
    ...
    /* Removes elements in order and adds them to a ArrayList, so long as the provided
       Comparable object is equal to their keys.  As soon as this is not true, the ArrayList is returned.
       You may provide a ArrayList -- putInHere -- to be filled in. */
    ArrayList extractMin(Comparable comparable, ArrayList putInHere)
        {
        if (putInHere == null) putInHere = new ArrayList();
        while( true )
            {
    ...
    /** Removes all key-equal minimum elements and adds them to a ArrayList, which is then is returned.
        You may provide a ArrayList -- putInHere -- to be filled in. */
    public ArrayList extractMin(ArrayList putInHere)
        {
        Comparable min = getMinKey();
        if (min==null)
            {
            if (putInHere == null) return new ArrayList(0);
