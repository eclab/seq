...
The MIDI Menu is where you set up MIDI to play and record.  First, you with {\bf Set MIDI Devices}, you can set up to 16 {\bf Outputs} and up to four {\bf Inputs}.  Each Input or Output is a combination of a MIDI Device and a Channel.  Once you have set up an input or output. it appears as an option in the features of Motifs which input or output MIDI.  Inputs and Outputs are saved to preferences so you don't have to set them up each time.  You can reset from preferences by pressing the {\bf Reload} button in the setup window.
