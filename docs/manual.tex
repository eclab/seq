...
\\
\\
\large\rm {\bf Seq Version 10}\\
\large\rm {\bf Manual Version 10.0}\\
\large\rm  July 2026\\
...
\small 
\noindent {\Large\bf Copyright }  2024--2026 by Sean Luke.

\vspace{0.25in}
\noindent {\Large\bf Thanks to } Filippo Canovalini and Cesar Liendo.
...
Seq was largely written from 2023--2026 by Sean Luke with help from Filippo Carnovalini and Cesar Liendo.

\paragraph{Who is Seq For?}

Seq was developed for research purposes: a major goal is to provide an environment where we could insert modules that automatically generate music or automatically filter and modify it.  We also wanted an architecture that was easily modified, so for example an AI optimizer might rip apart an entire song and restructure it into a different form.  Seq doesn't have those kinds of extensions in its public distribution, but stay tuned.

So if you're not an academic, while use Seq?  Seq's approach to building songs is very unusual.  It has disadvantages to be sure: compared to a linear arranger in a DAW, it doesn't at present show clips side-by-side in a timeline, so you have to do some math in your head.  But Seq makes it easy to reuse and modify pieces of music in unusual ways to form a whole composition: and Seq has significant performance facilities.  We think Seq would be of particular interest to modular synthesizer composers and musicians, to composers and producers of ambient music, EDM, IDM, electronica, long-form music, minimalist music, and other music involving high degrees of experimentation, or reuse and remixing.  Though it's computer software, Seq might be of significant interest to DAWless setups.  But in fact we use Seq for all sorts of music.

\paragraph{Seq is Hierarchical}
...
\subsection{Where We're Going with Seq}

Seq's sequence structure is very amenable to customization and automated modification.  Our ultimate goal is to ask what kinds of machine learning, optimization, or search techniques we could add to Seq to convert it into a {\bf co-creative tool} for the composer, that is, a tool that works with the composer, bouncing ideas back and forth, to produce a song.  We have previous experience in tools of this sort (see Sean Luke's {\bf Edisyn} for example).  To this end, we'll be adding motifs which use AI or other tools to generate automated music, and also motifs which take music and modify it automatically in interesting ways.

We'd also like to see a Live Coding module for Seq, though there are some challenges due to the poor options among Live Coding programming languages.

In the immediate future, we'll be adding external sync, interface improvements, and more generative algorithms.
...
Excluding Macros and Macro Children, there are presently eleven Motifs available in Seq:

\begin{itemize}
\item Four {\bf Basic (Note-Generating) Motifs}: Step Sequence, Notes, Generator, and Silence.  These do not have children Motifs, except for Generator, which has an optional child Motif.
...
A quick note about {\bf System Exclusive} (or {\bf Sysex}) events.  These non-note events do not have values.  Instead they are associated with a textual string of {\bf hexadecimal values}, each separated by whitespace.  We assume you understand how sysex works and so what these strings look like.  Because the events do not have values, we assign them random vertical and horizontal placement in the display so multiple Sysex events at the same timestamp don't pile up on top of one another.
...
All notes and events are by default emitted out the same MIDI output specified by the {\bf Out} in the Notes Inspector.  However you can customize the {\bf Out} value on a per-note or per-event basis.  If you do this, the color of the note or event will change to indicate that it is not the default value.
...
You select a Step by Command- or Alt- (or Option-)clicking on it.  This also selects the Step's Track.  You can also select a different Track (and its equivalent Step) by clicking on the Track Header (see below).  When you select a Step and Track, they will show their respective Inspectors at right below the primary Step Sequence Inspector.

You can toggle a Step on or off, by clicking on the step. This also selects it.  Thus another way to select a Step (but not toggle it) is to double-click on the Step, which effectively toggles it twice and selects it.

If you triple-click on a Step, you will clear all the Steps in its Track.
...
Seq can fill a track with a Euclidean Rhythm with {\bf Euclid}.  You specify \(N\), the number of steps you want on (the rest will be off.  \(K\) is just the length of the track.  You can also {\bf Rotate} your Euclidean Rhythm to a new starting point  The Rhythm is generated when you press {\bf S} (for Set).  You can also generate a totally random Rhythm by pressing {\bf R} (for Random).
...
Under the {\bf Randomize} submenu are six options for randomizing the pattern of the current selected track or of all tracks.  They're set up so you can easily select them with keystrokes.
...
You add Children to the Automaton by dragging them in from the Motif List.  You add Decision Points to the Automaton by dragging them in from one of the seven Decision Point buttons at top ({\bf Delay, Chord, Finished, Iterate, Random, Fork, Join} and {\bf Trigger}).  You can delete a Node by selecting it, then pressing the {\bf Delete Button} at bottom, and similarly you can duplicate a Node by selecting it, then pressing the {\bf Duplicate Button}.  You can move nodes to different slots by dragging them. 
...
\item{\bf Trigger}\quad This Decision Point emits a Thread when it receives a {\bf trigger} a certain Parameter (by default Parameter 7, but you can change that).  If {\bf Wait For Thread} is checked, then it must also have received a Thread prior to the trigger; if multiple Threads were received prior to the trigger, only one is emitted: the others wait to be emitted in successive triggers.  You can use this in combination with MIDI CC in a parent Modulation motif to trigger a Thread when you press a remote button on a synthesizer, for example.
...
\item {\bf Step Seq}\qquad The argument is modulated over time using a {\bf parameter step sequencer} with up to eight stages.
\item {\bf MIDI CC}\qquad The argument is modulated over time using external MIDI CC parameter data.
...
\item {\bf Phase / Var}\qquad For the {\bf Random} and {\bf Sample and Hold (S\&H)} wave types, this defines the {\bf Variance}, that is, how wide a swing in randomness is used when computing the next target location.  For all other wave types, this defines the {\bf initial phase} of the wave, ranging from 0.0 to 1.0.
...
This is a step sequence.  It's much like a simplified Envelope with Hold: where each stage has the same duration.   We call these stages {\bf steps}.
...
This takes incoming MIDI CC data and outputs it as the parameter value.  There are 128 possible CC parameters, and each can be set to one of 128 values.  The MIDI CC modulation function receives MIDI messages of one CC parameter, which you specify, and maps its values (0...127) into parameter values from 0.0 to 1.0 inclusive.  To do this, you'll need to specify the device from which the MIDI CC messages are received, the specific CC message parameter of interest, and a {\it default value}.  The default value is what's output by the MIDI CC modulation function when it has not yet received a CC message of the given parameter since the start of the sequence.

\begin{itemize}
\item {\bf In}\qquad The MIDI Input Device, or {\it None} (meaning that incoming MIDI CC data is ignored). 
\item {\bf CC}\qquad The CC parameter, one of 0...127.  Only data arriving on this MIDI CC parameter will be mapped.
\item {\bf Default}\qquad The value to map if we have not received any data on the MIDI CC parameter since the sequence began playing.
\end{itemize}

Note that this isn't the only way you can map incoming MIDI CC data to a Seq parameter.  You can also do it via the {\bf Dial Parameters} in the {\bf Select} motif.  However, the Dial Parameters override the Select's {\it own parameters}, whereas the MIDI CC Modulation Function here will override only the parameters of the child motif to the Modulation.
...
\begin{figure}[t]
\centering
\includegraphics[width=6.5in]{Generator}
\vspace{-2em}
\caption{Generator Motif.}
\label{arpeggio}
\end{figure}

\subsection{Generator}

Generator is where Seq runs various algorithms which perform generative music, that is, they automatically generate melodies or rhythms, either on their own or in response to MIDI from a child (such as to accompany~it).

At present Generator has two algorithms:

\begin{itemize}
\item {\bf Triadex Muse}, from a 1969 music-generating device by Marvin Minsky and Edward Fredkin of MIT and Triadex Inc.  The Muse uses two logic devices (a binary counter and a linear feedback shift register or LSFR) to generate random melody patterns.
\item {\bf Grids}, from a 2013 Eurorack module by {\'E}milie Gillet of Mutable Instruments.  Grids interpolates among a wide range of three-drum patterns in a two-dimensional space, plus additional parameters for complexity and randomness.
\end{itemize}

\noindent You select the algorithm from the {\bf Algorithms} popup menu at right.  The main part of the motif then has three parts:

\begin{itemize}
\item {\it (At Left)} The algorithm's parameters.  The image above shows the parameters for the Triadex Muse.
\item {\it (At Right)} Instructions for using the algorithm and all of its parameters.
\item {\it (At Top}) A slot for an optional single motif Child.  Some algorithms may operate by receiving MIDI from this child, and then emitting MIDI to accompany it in some way, and so require a child.  Many other algorithms generate music on their own and so will simply ignore any child you place here. 
\end{itemize}

\paragraph{Additional Parameters}

\begin{itemize}
\item {\bf Out}\quad Specifies the MIDI Device that the algorithm will use to emit its generated music.  This can be any device, or set to be the {\bf Same as Received}.
\item {\bf Received}\quad  When the Child emits MIDI for the algorithm to accompany, the algorithm will expect MIDI sent to this device only.  Other MIDI will be ignored.
\item {\bf Omni}\quad  If checked, then when the Child emits MIDI for the algorithm to accompany, the algorithm will respond to MIDI sent to any device (regardless of the setting of {\bf Received}).
\item {\bf Pass}\quad  If checked, then when the Child emits MIDI, the algorithm will then pass this MIDI to its parent motif in addition to any MIDI it generates.  this MIDI the algorithm will respond to MIDI sent to any device (regardless of the setting of {\bf Received}).
\item {\bf Max End Time}\quad Specifies the time that the algorithm will stop playing.  The algorithm is free to play for a shorter period of time than this.
\end{itemize}

Whenever the Generator motif is started, or restarted, it will reset the algorithm to its initial position and then play from there.
...
\paragraph{CC Control via the Modulation Motif}
Note that this isn't the only way you can map incoming MIDI CC data to a Seq parameter.  You can also do it via the {\bf MIDI CC} modulation function in the {\bf Modulation} motif.  However, unlike the Select motif, where the CC messages override the Select's own parameters, in the Modulation motif the CC messages override the parameters of the Modulation mofit's child.
