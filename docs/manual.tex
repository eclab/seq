...
We'll describe these next in detail.  More modulators are planned for the future.  But first a word on mapping.
...
\item {\bf Phase}\qquad The initial phase of the wave, ranging from 0.0 to 1.0.
...
This indicates that the modulation function for this argument (let's call it \(m\) is the same as the one for some other argument \(n\). Note that \(n\) must be {\bf less than} \(m\).  If \(n \geq m\), then Same acts just like {\bf None}.  Mapping still occurs regardless.
