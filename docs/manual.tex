...
\\
\\
\large\rm {\bf Seq Version 6}\\
\large\rm {\bf Manual Version 6.0}\\
\large\rm  August 2025\\
...
\noindent You have the Control Out set to your synthesizer device.  Instead, set it to your LaunchPad.
...
You can play more than one note in the same step: just click multiple squares.  You can also play a rest: just have a column with no squares selected in it.

Finally, you can make {\bf ties} to extend the length of notes.   If you shift-click on a pattern note, you will make a tie.  A tie indicates that the immediately previous note will be lengthened and continued.  If there is no immediately previous note (it's a rest), then the tie has no effect.  Ties are blue.
...
At present, Filter has five MIDI Effects available:
...
\item {\bf Drop}\qquad With some probability, drops a note (don't let it pass through).
\item {\bf Parameter}\qquad Changes the value of a non-note parameter such as pitch bend, CC, or aftertouch.
\end{itemize}
...
\item {\bf Length}\qquad You can set the length of all notes.
\end{itemize}
...
This effect randomly deletes notes.

\begin{itemize}
\item {\bf Probability}\qquad The probability that a note would be deleted.
\end{itemize}



\paragraph{The Parameter Effect}
